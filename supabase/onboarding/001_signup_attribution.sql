...
--
-- APPLIED. Verified live 2026-08-07 by querying the Data API directly: the
-- table, its columns and its grants all answer on prod. This header previously
-- read "NOT YET APPLIED" and was simply stale — Adrian applied it by hand and
-- the note was never updated. Hand-applied migrations do not show up in
-- `list_migrations`, so the file's own comment is the ONLY status record, which
-- is exactly why it went stale. Verify against the schema, never this comment.
--
-- Nothing in the app writes this table yet — attribution is captured on the
-- anonymous device session and there is no account to attach it to until auth
-- is wired at the paywall (`startTrial()` in
-- `components/onboarding/screens/paywall.tsx`). The table is waiting on that
