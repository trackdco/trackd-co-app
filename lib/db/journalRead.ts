...
/**
 * THE JOURNAL READ: every entry with its markers and signed photos, and the
 * markers this person can add. One definition, used by the Progress page and by
 * Home's in-place journal (build-brief-final §3.5), so the two can never offer
 * different markers or show a different entry for the same day.
 *
 * Server-side only (it takes the request's Supabase client; RLS scopes every
 * row to the signed-in user). Moved verbatim from `app/(app)/progress/page.tsx`.
 */
export async function readJournal(
  supabase: SupabaseClient,
  userId: string,
): Promise<{ entries: JournalEntry[]; options: MarkerOption[] }> {
  const [
    { data: profile },
    { data: markerData },
    { data: entryData },
    ...
    // `sex` decides which markers are offered (Spec 04), read raw.
  ...
  return { entries: journalEntries, options: markerOptions };
}
