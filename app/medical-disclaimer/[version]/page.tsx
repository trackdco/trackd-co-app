...
/**
 * A SUPERSEDED VERSION OF THIS DOCUMENT, AT A STABLE URL — /medical-disclaimer/1.3.
 *
 ...
 *
 * ⚠️ THIS DOES NOT DECIDE WHAT IS CURRENT. /medical-disclaimer keeps that job and is
...
export async function generateMetadata({
  ...
  return {
    title: `Medical Disclaimer v${version} · Trackd Co`,
    /**
...
export default async function MedicalDisclaimerVersionPage({
  params,
}: {
  params: Promise<{ version: string }>;
}) {
  const { version } = await params;
  return <LegalDocument docType="medical_disclaimer" version={version} />;
}
