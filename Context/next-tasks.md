...
Last updated: 2026-08-30 (deep links now survive sign-in at all five doorways;
G2 CANCELLED by D113; the webhook pile is empty; one UNVERIFIED trial-conversion
risk still open; BOTH environment blockers from 29 Aug are CLEARED — the clean
`npm ci` removed the 19 duplicated `@types … 2` folders, `npm run check` now runs
all four steps and passes, and the tree builds; the Safari handoff sign-in is
built and pushed but held off `main` pending one walk on a handset)

---

## 🟡 SAFARI HANDOFF SIGN-IN — BUILT, PUSHED, WAITING ON A HANDSET

Branch `safari-handoff-login` (2 commits, 3 files). **Deliberately not merged** —
Adrian asked for it held while he was away. What it does and how the Chrome-only
gate works is in `progress-tracker.md`; the review artifact is
`30701f61-ddce-4aa6-8e2a-9f597a4f2db7`.

- [ ] **Walk it on a real iPhone**: Chrome → install step → Share → Open in
      Safari → sign in → land back on the install step. This is the one thing a
      desktop browser cannot answer, because it needs Chrome's actual share
      sheet handing the URL over to the actual Safari.
- [ ] Merge to `main` once that walk is clean. Everything else is verified:
      `npm run check` (all four steps), `next build`, and both `/login` variants
      plus the stamp rendered from the running app.

**If the copy fires when it shouldn't**, the marker is the thing to look at:
`from=chrome` is stamped by `install.tsx`, carried by `app/onboarding/page.tsx`,
required by `app/login/page.tsx`. All three must agree.

### Still on the text list, not the drawn walkthrough

Firefox and Edge on iOS. Their share sheets are laid out differently from
Chrome's and were never drawn, so `installFlowId` leaves them with the words.
Low value unless someone actually turns up in one.
