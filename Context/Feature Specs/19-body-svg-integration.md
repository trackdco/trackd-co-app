...
3. The original spec: `Context/Feature Specs/19-SiteRotation-Rework.md`.
...
**Where they actually landed:** `Context/Feature Specs/svgs/body-svg/male/im_front.svg`
and `im_back.svg` (plus the `female/` set) — note the `svgs/` segment and the
underscores, which differ from the guess above.
...
- Verification renders: `Context/Feature Specs/svgs/body-svg/_verify/` (labeled front/back,
