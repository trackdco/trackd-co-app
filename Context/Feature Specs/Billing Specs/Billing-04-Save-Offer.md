...
**⚠️ SHIP-TOGETHER PAIR — this spec and `07-notifications.md` ship together or not at
all.**

The offer's terms line ends "and we'll remind you first." That is a promise, and
`07` is what keeps it. **Per the amended D1: the release condition is a reminder
verifiably firing before a courtesy charge, proven on a Stripe test clock.** The
in-app banner and push are the carrier; Stripe's own email is supplementary and is
explicitly not the backstop, because its reminder is configured seven days before a
trial end and a trial courtesy period is seven days long. **If that reminder cannot
be proven to fire, the sentence does not ship, and neither does this spec.** Q79 is
...
- [ ] **A reminder verifiably fires before a courtesy charge, proven on a test
      clock.** Until this is observed, the terms line's final clause does not ship
      and neither does this spec.
...
It does not block construction, but **the pair's release depends on a reminder being
observed firing before a courtesy charge**, per the amended D1.
