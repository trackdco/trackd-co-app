...
`OPEN: awaiting answer to Q76` — the exact minimum Stripe enforces, so the
constant can be named rather than discovered at runtime.
...
The new field is `graceEndsAt`: the ISO instant a live beta grace ends, or null.
...
- `OPEN: awaiting answer to Q76` — Stripe's minimum accepted `trial_end` offset, so
  §3.4's clamp constant can be named.
- `OPEN: awaiting answer to Q74` — what `planLabelFor` returns today for a
  beta-grace account with no subscription, so `08-billing-screen.md` can decide what
  a mid-grace subscriber reads. Blocks `08`, not this spec.