...
~~**⚠️ UPDATED 27 Aug 2026 — next free decision number: `D114`. Next free question
number: `Q108`.**~~ **D114** was taken 2026-09-03 for the
one-deletion-flow-for-everyone ruling.

**⚠️ UPDATED 3 Sep 2026 — next free decision number: `D115`. Next free question
number: `Q108`.** D114 was taken on the founder's instruction to take the LOWEST
...
a build session resolves by claiming it. So the lowest free number and the next
free number are NOT the same today, and a future session should read this before
assuming either. **D113** was taken for the beta-grace HOLD (below). `D112` was
never taken and remains free; it was the next-free marker on 26 Aug and nothing
claimed it. Adrian declined to name a number and asked for the next free to be
taken and reported, which it was.
...
| D113 | **`POST /api/billing/beta-grace` is NOT to be re-run. A comp-list member who is refused gets a SINGLE-ROW, hand-applied grant instead** | 06, 12 | Adrian, 27 Aug 2026, with the gate ON. ⚠️ **A DECISION NOT TO RUN A ROUTE — the documented repair was DECLINED, not forgotten.** `next-tasks.md` and `004`'s own header both name re-running this route as the repair path for a comp-list member holding no row; that path is now closed and this row is why. **Two findings, both read from source, neither requiring a run.** **(1) It MODIFIES existing rows.** `route.ts:239-256` is a real `UPDATE ({active_until: null})`, not an insert, guarded by `grant.kind === "comp" && timeLimitedComp.has(account.id)`. It fires for zero accounts on today's data — every comp-list member with an account is already undated — but that is a fact about the data, not a property of the code. **(2) THE STRONGER REASON — it computes the date AT RUN TIME.** `const now = new Date()` (`route.ts:126`) feeds `grantExpiry(grant, now)` (`route.ts:277`), which returns `now + 14 days`. It reads no fixed value and cannot be made to. A run tonight writes an instant ~14 days from tonight, **not** `2026-09-10T04:00:11.374343+00` — putting a SECOND ending instant in the table, which `004`'s own VERIFY block forbids in as many words: *"Two different instants means it was run twice against different row sets, and the notice can then only be right for one of the groups."* That date is what 82 real people will be shown in writing. **Population: one.** `angusbrake6@gmail.com` is the only comp-list address with no auth account, and no comp-list address holds an account without a row. He cannot be repaired today — `entitlements.user_id` needs a `profiles` row and he has none — so the repair is a STANDING INSTRUCTION conditional on his signing up. ⚠️ **His grant is `active_until = NULL`, never the 004 instant**: a comp-list member is free FOR LIFE, and `004`'s header already names dating one of those as the mistake it refuses to make. See D113a |
| D113a | The `239-256` write is BROADER THAN ITS PREDICATE, and `barbrake24+test1@gmail.com` is safe only by the ABSENCE of plus-address canonicalisation | 06 | Recorded 27 Aug 2026 alongside D113. Both are LATENT on today's data and neither is a bug today. **(a)** The predicate tests one specific shape — comp-list member whose ONLY entitlement is a dated `comp` row — but the write filters on `user_id` + `source='comp'` with **no `product` filter and no `active_until` guard**, so it clears the expiry on EVERY comp row that user holds. Each user holds at most one today. **(b)** `barbrake24+test1@gmail.com` **is one of the 82 rows at the 004 instant.** `betaGrantFor` normalises with `.trim().toLowerCase()` then does exact `COMP_EMAILS.includes()` — no plus-address folding — so it reads as `grace` and the upgrade branch cannot reach it. Add the obvious canonicalisation and a row at the 004 instant becomes writable by (a). ⚠️ Written as warnings AT BOTH SITES (`betaGrace.ts`, `beta-grace/route.ts`), not only here, so somebody adding a normalisation trips over it |
