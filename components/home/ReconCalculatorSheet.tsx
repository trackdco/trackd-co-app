...
interface ReconCalculatorSheetProps {
  open: boolean
 ...
 */
export function ReconCalculatorSheet({
  open,
  onOpenChange,
}: ReconCalculatorSheetProps) {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        <div
          ref={cardRef}
          style={cardStyle}
          className="flex max-h-[92dvh] flex-col overflow-y-auto rounded-t-3xl border-t border-border-default bg-bg-surface shadow-lg"
        >
          <div
            {...handleProps}
        ...
        <SheetDescription className="sr-only">
          Work out the concentration and the volume to draw per dose from the
          powder amount and the bacteriostatic water you add.
        </SheetDescription>

        <div className="space-y-5 px-5 pb-[calc(env(safe-area-inset-bottom)+1.5rem)] pt-1">
          {/* Inputs */}
          <NumberField
