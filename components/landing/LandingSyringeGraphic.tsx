...
import { graduations, type SyringeSize } from "@/lib/calculator/syringe"

/*
 * THE PUBLIC SITE'S SYRINGE, frozen as it shipped (final design build,
 * 2026-09-26). The app's syringe was redrawn (`components/calculator/
 * SyringeGraphic.tsx`, build-brief-final §3.13) with new geometry, and the
 * public site is to stay as it is apart from the app previews inside it. So the
 * public calculator and Kyle's closer draw this copy, on the geometry they
 * shipped with, while the Features preview (an app screen) shows the new one.
 */
const VIEW_W = 320
const VIEW_H = 66
const BARREL_X = 62
...
const BARREL_H = 28
const BARREL_R = 3
const AXIS_Y = BARREL_Y + BARREL_H / 2
const TICK_MINOR = 6
const TICK_MAJOR = 11
const TICK_ANCHOR = 4
const LABEL_Y = BARREL_Y + BARREL_H + 16
const LABEL_SIZE = 10
const barrelX = (fraction: number) => BARREL_X + BARREL_W * fraction
 ...
 */
export function LandingSyringeGraphic({
  size,
  fill,
  label,
}: {
